...
EntityPlayer = tpf.Entity.extend({
	...
	update: function() {
		...
		// Perhitungan kecepatan baru berdasarkan velocity; update sector and light etc...
		this.parent();
		...
		// Update posisi kamera dan view angle
		var cx = this.pos.x + this.size.x/2,
			cy = this.pos.y + this.size.y/2;
	...
	receiveDamage: function( amount, from ) {
		...
		var xedge = ig.game.hud.width;
		var ypos = a < 0 ? ig.game.hud.height/3 : 0;
		var xpos = Math.abs(a).map( 0, Math.PI, -xedge, xedge );
		
		ig.game.hud.showDamageIndicator( xpos, ypos, 3 );
	...
	kill: function() {
		ig.game.hud.showMessage('Kau Mati!', tpf.Hud.TIME.PERMANENT);
